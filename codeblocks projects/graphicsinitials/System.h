...
class System
{
   ...
   Particle particles[MAX_PARTICLES]; //initializes and array of type Particle
public:
   System(void);
   virtual ~System(void);
   void createParticle(Particle *p); //creates and initializes a single particle
   void createParticles(); //calls createParticle() to initialize all particles in system
   void updateParticles(); //updates particles according to forces being used
   void turnToSmoke(Particle *p); //called only during fire with smoke system to turn dead fire into smoke
